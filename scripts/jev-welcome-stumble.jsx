// Run from the repository root: higgsedit build scripts/jev-welcome-stumble.jsx
...
export default async ({project}) => {
  const p = await project({dir:"build/jev-welcome-stumble",size:"800x340",fps:25,background:"#0d1423"});
  const buddy = await p.add("docs/assets/jev-character.png");
  const star = await p.add("docs/assets/jev-star.png");
  // Two off-balance steps, a recovery, then eye contact and a wink.
  const entrance = [
    track("offsetX",[[0,-140],[0.42,-40],[0.68,-10],[0.95,25],[1.18,13],[1.42,-16],[1.65,-8],[1.9,4],[2.18,0],[6,0]]),
    track("offsetY",[[0,330],[0.3,38],[0.45,15],[0.65,-14],[0.96,14],[1.16,-8],[1.42,17],[1.67,-5],[1.92,5],[2.2,0],[5.45,0],[5.95,330]]),
    track("rotation",[[0,-28],[0.43,-12],[0.68,8],[0.97,24],[1.18,7],[1.43,-21],[1.67,9],[1.92,-4],[2.2,0],[3.55,0],[3.85,-5],[4.4,0],[6,0]]),
    track("scaleX",[[0,0.65],[0.3,0.89],[0.45,1.12],[0.65,0.95],[0.96,1.08],[1.16,0.96],[1.43,1.1],[1.67,0.96],[1.92,1.03],[2.2,1],[5.45,1],[5.95,0.65]]),
    track("scaleY",[[0,0.78],[0.3,1.07],[0.45,0.82],[0.65,1.04],[0.96,0.9],[1.16,1.03],[1.43,0.84],[1.67,1.04],[1.92,0.97],[2.2,1],[5.45,1],[5.95,0.78]])
  ];
  p.compose([
    ...
    <rect x={95} y={281} width={140} height={10} radius={5} fill="#030812" animate={[
      track("opacity",[[0,0],[0.3,0],[0.45,0.35],[1.43,0.35],[2.2,0.23],[5.45,0.23],[5.95,0]]),
      track("scaleX",[[0,0.4],[0.45,1.1],[0.65,0.8],[0.96,1],[1.43,1.1],[1.67,0.9],[2.2,1],[6,1]])
    ]}/>,
  ...
  await p.frame(3.92,"renders/jev-welcome-stumble.png");
  await p.render("renders/jev-welcome-stumble.mp4",{concurrency:2,shards:2});
};
